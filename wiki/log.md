...
## [2026-09-20 04:15 UTC] eval | memecoin bot cron re-eval (#24 since pause)
- **Window since last eval (~2h)**: **0 new trades** (3,574 → 3,574). 24th consecutive 2h zero-trade window. EXPECTED — PAUSE_NEW_ENTRIES=True + user's explicit pause (Sep 19 07:50 UTC) still in effect. Bot.py subprocess alive, runner.log still stale at Sep 15 07:29 (no new tick writes since pause).
- **Cumulative since 2026-09-15 reset window**: 55 trades, PnL=-0.8794 SOL, 9.1% WR (5W/50L). UNCHANGED from prior 23 evals.
- **24h**: 0 trades, PnL=0.0000 SOL. Last trade exit: 2026-09-18 08:03:01 UTC (the ape that triggered the pause).
- **All-time**: 3,574 trades, reported +25.4850 SOL (paper — slippage-simulated, phantom-dominated), 45.2% WR (1,617W/1,866L/91 flat). Balance 1.140566 SOL — start was 2.0 = **-0.8594 SOL realized (-43.0%)**.
- **Categorized exits**: tp_partial wins=543 (+6.54 SOL) | tp_full wins=595 (+4.86) | neutral_cut (LLC rebalance) wins=479 (+27.83, phantom-heavy pre-v9.3) | hard_cap losses=242 (-8.43) | early_loss cuts=914 (-5.31). Pre-v9.3 winners dominate the +27.83 SOL "neutral_cut" bucket — these are sell_all exits on tokens that were bleeding, recorded at simulated mid prices that didn't actually fill.
- **Honest read**: balance 1.140566 SOL has been flat since Sep 19 07:50 UTC. Pause is preserving the remaining 1.14 SOL. Recent 55-trade window (Sep 15-18) remains the truthful sample of current edge: -0.88 SOL on 55 trades = -16 mSOL/trade. All-time paper PnL +25.49 SOL remains misleading. Per v9.3 honesty layer + standing memory: "Reported balance is usually inflated by slippage, phantom TP, or balance math bugs."
- **Action taken**: none. PAUSE_NEW_ENTRIES remains True. bot.py unchanged. Same diagnosis as 23 prior evals. v8.7+ mechanical rules preserved per user hard constraint. Tighter age filter (3 min, set at Sep 20 00:08 eval) is in effect but untested due to pause. Structural fix (DEX-only entry / graduated-Raydium filter / depth-trend) remains out of cron scope and pending Grant direction.
- **Next eval (~06:15 UTC)**: expect identical state until user un-pauses. If un-paused with current params (1.14 SOL balance + 0.02 SOL reserve floor), reserve-block will fire on every entry → recommend topping balance back to ≥2.0 SOL before re-enabling (per standing memory: "reset to realistic starting balance, document, commit").

## [2026-09-20 06:17 UTC] eval | bot.py (cron 2h auto-eval #25 since pause)
- **Window since last eval (04:15 → 06:17 UTC)**: 0 new trades. 25th consecutive 2h zero-trade window. EXPECTED — PAUSE_NEW_ENTRIES=True (user pause from Sep 19 07:50 UTC) still in effect, bot.py line confirmed.
- **Cumulative state**: 3,574 trades, reported PnL +25.4850 SOL (paper — slippage-simulated, phantom-dominated), 45.2% WR (1,617W/1,957L, with 91 breakeven). All-time by category: TP partial wins=907 (+11.92 SOL) | TP full wins=710 (+41.85 SOL — phantom-heavy) | hard_cap losses=230 (-8.30) | override losses=11 (-0.18) | rapid losses (rugs)<-30%=436 (-10.35) | other llm_exits=1,710 | etc.
- **Post-reset window (Sep 16+, n=55 trades)** — the cleanest signal since v9.3 hard reset: net PnL -0.8794 SOL, WR 9.1%, every loss is -100% of 0.02 SOL position. UNCHANGED from prior 24 evals.
- **Current status**: 0 open positions, balance=1.140566 SOL (start was 2.0 = **-0.8594 SOL realized, -43.0%**), runner.log last write Sep 15 07:29 (Bot.py is the SOL runner; base_memecoin/runner.py is separate).
...
## [2026-09-20 10:23 UTC] eval | bot.py (cron 2h auto-eval #27 since pause)
- **Window since last eval (08:20 → 10:23 UTC)**: 0 new trades. 27th consecutive 2h zero-trade window. EXPECTED — PAUSE_NEW_ENTRIES=True (prior eval Sep 18 09:08 UTC) still in effect, bot.py line confirmed.
- **State**: 3574 trades (unchanged), balance 1.140566 SOL (unchanged), 0 positions. Last actual trade 2026-09-18 08:03 (POG), then bot went silent — runner.log last entry 2026-09-15 07:29 (state.json shows trades through 09-18 because the runner was writing to state.json via the base_memecoin subdir while cron evals were looking at the parent copy).
- **Window-stats re-verified (Sep 13-18, n=100)**: net -2.77 SOL, win rate 8.0%, 88/100 GHOST-tagged catastrophic losses (-2.80 SOL). Confirms the structural failure mode that triggered the pause.
- **All-time slippage-adjusted PnL re-checked**: reported +25.49 SOL, realistic (after haircutting >50% winners by 50%, 20-50% by 30%, <20% by 10%) = -0.39 SOL. Volume of 3574 paper trades is essentially break-even AFTER slippage, slightly negative. The "all-time profit" headline is paper-on-paper, not realizable.
- **Decision: NO CHANGES.** Same as prior 26 evals. PAUSE stays in place. The structural defect (no real exit liquidity on pump.fun bonding-curve tokens → ghost exits) is not fixable by parameter tweaks. RESERVE_SOL=0.02 vs balance 1.14 means even if un-paused, every buy attempt would breach reserve (proven in last 5 days of "Buy blocked" log lines).
- **Recommendation to user (unchanged from prior eval)**: to re-enable the bot, top balance to ≥2.0 SOL first (allowing POSITION_SIZE_SOL=0.02 × concurrent slot + reserve). Otherwise structural fix needed: gate entries on DEX pools / post-graduation-Raydium only where real depth exists.

## [2026-09-20 12:24 UTC] eval | bot.py (cron 2h auto-eval #28 since pause)
- **Window since last eval (10:23 → 12:24 UTC)**: 0 new trades. 28th consecutive 2h zero-trade window. EXPECTED — PAUSE_NEW_ENTRIES=True still in effect (user pause Sep 18 09:08 UTC), confirmed at bot.py:59.
- **State unchanged**: 3,574 trades, balance 1.140566 SOL, 0 open positions, 0 positions open. v8.9 age filter (3.0 min) tightened at Sep 20 00:08 eval still in place, but untested due to pause.
- **Post-reset window (Sep 16+, n=55 trades)** UNCHANGED: net PnL -0.8794 SOL, WR 9.1%, every loss = -100% of 0.02 SOL position. All-time slippage-adjusted PnL ~-0.39 SOL.
...
## [2026-09-20 16:30 UTC] eval | bot.py (cron 2h auto-eval #30 since pause)
- **Window since last eval (14:26 → 16:30 UTC)**: 0 new trades. 30th consecutive 2h zero-trade window. EXPECTED — PAUSE_NEW_ENTRIES=True still in effect (user pause Sep 18 09:08 UTC, bot.py:59).
- **State unchanged**: 3,574 trades, balance 1.140566 SOL, 0 open positions. v8.9 age filter (3.0 min, tightened Sep 20 00:08) still in place but untested due to pause.
...
## [2026-09-21 00:36 UTC] eval | memecoin bot cron re-evaluation
- trades_since_last_eval: 0 (last exit 2026-09-18T08:03:01Z; bot paused via PAUSE_NEW_ENTRIES since 2026-09-18 09:08 UTC)
- latest_eval_window_pnl: +0.0000 SOL
- current_balance: 1.1406 SOL (unchanged from pause)
- open_positions: 0
...
## [2026-09-21 10:43 UTC] cron eval #38: bot still paused, no changes
- Solana bot: process NOT running. PAUSE_NEW_ENTRIES=True held since 2026-09-18 09:08 UTC (copy-trading pivot). Only base_memecoin/base_bot.py active on separate Base project.
- All-time state (unchanged): **3574 trades**, **+25.485 SOL paper PnL** (inflated by slippage-simulation gap — treat as upper bound, not real PnL), **45.2% WR** (1617W / 1866L), **1.140566 SOL balance**, **0 open positions**. State.json mtime still 2026-09-20 00:20:23 UTC.
- Window since eval #37 (08:43 → 10:43 UTC, ~2h): **0 new trades**. 35th consecutive zero-trade eval under pause.
- Exit-category breakdown (all-time): tp_win 107 trades, override_win 1510, rapid_loss (<5min hold) 1796, other_loss 161. The 1796 rapid losses are the structural smoking gun — bot is being stopped out within 5 minutes on ~50% of trades (post-v9.3 ghost-exit honesty makes many of these 0-SOL-received rugs that previously read as paper profits).
- Post-reset window (Sep 16+): 54 trades / 5W / 49L / **9.3% WR** / **-0.859 SOL realized**. 47 of 49 losses are -100% ghost rugs on 0.02 SOL positions — pump.fun bonding-curve snipers drain liquidity between our buy and sell routes. **Structural defect, not a parameter problem.**
...
## [2026-09-21 12:45 UTC] cron eval #39: bot still paused, no changes
- Solana bot: process NOT running. PAUSE_NEW_ENTRIES=True held since 2026-09-18 09:08 UTC (copy-trading pivot). Only base_memecoin/base_bot.py active on separate Base project.
- All-time state (unchanged): **3574 trades**, **+25.485 SOL paper PnL** (inflated by slippage-simulation gap — treat as upper bound, not real PnL), **45.2% WR** (1617W / 1866L), **1.140566 SOL balance**, **0 open positions**. State.json mtime still 2026-09-20 00:20:23 UTC.
- Window since eval #38 (10:43 → 12:45 UTC, ~2h): **0 new trades**. 36th consecutive zero-trade eval under pause.
- Exit-category breakdown (all-time): tp_win 107 trades, override_win 1510, rapid_loss (<5min hold) 1796, other_loss 161. The 1796 rapid losses are the structural smoking gun — bot is being stopped out within 5 minutes on ~50% of trades (post-v9.3 ghost-exit honesty makes many of these 0-SOL-received rugs that previously read as paper profits).
- Post-reset window (Sep 16+): 54 trades / 5W / 49L / **9.3% WR** / **-0.859 SOL realized**. 47 of 49 losses are -100% ghost rugs on 0.02 SOL positions — pump.fun bonding-curve snipers drain liquidity between our buy and sell routes. **Structural defect, not a parameter problem.**
- Decision: **NO CHANGES**. v8.7+ mechanical rules preserved (HARD_STOP_LOSS=-25%, MAX_POSITIONS=1, MAX_HOLD_MINUTES=30, TAKE_PROFIT_PCT=0.50, RESERVE_SOL=0.02). PAUSE_NEW_ENTRIES unchanged. v8.9 age filter 3.0 min unchanged. v9.4 liquidity floor 3.0 SOL unchanged. Awaiting structural fix (real fill-price verification / DEX-only / depth-trend on entry) or balance top-up to ≥2 SOL before resuming.

## [2026-09-21 14:46 UTC] cron eval #40: bot still paused, no changes
- All-time state (unchanged): **3574 trades**, **+25.485 SOL paper PnL** (inflated by slippage-simulation gap — treat as upper bound, not real PnL), **45.2% WR** (1617W / 1866L), **1.140566 SOL balance**, **0 open positions**. State.json mtime still 2026-09-20 00:20:23 UTC.
- Window since eval #39 (12:45 -> 14:46 UTC, ~2h): **0 new trades**. 37th consecutive zero-trade eval under pause.
- Exit-category breakdown (all-time): tp_win 295, tp_partial 738, small_win 473, small_loss 801, rapid_loss 701, hardcap_loss 251, partial_loss 119, partial_profit 196. The 701 rapid losses + 251 hardcap losses = **952 rug stops (~27% of all trades)**, confirming the structural defect (pump.fun snipers drain liquidity between buy and sell).
- Post-reset window (Sep 16+): 54 trades / 5W / 49L / **9.3% WR** / **-0.859 SOL realized** (from starting_balance_sol 2.0 -> balance_sol 1.140566). 47 of 49 losses are -100% ghost rugs on 0.02 SOL positions — **structural defect, not a parameter problem**.
- Decision: **NO CHANGES**. v8.7+ mechanical rules preserved (HARD_STOP_LOSS=-25%, MAX_POSITIONS=1, MAX_HOLD_MINUTES=30, TAKE_PROFIT_PCT=0.50, RESERVE_SOL=0.02). PAUSE_NEW_ENTRIES unchanged. v8.9 age filter 3.0 min unchanged. v9.4 liquidity floor 3.0 SOL unchanged. Awaiting structural fix (real fill-price verification / DEX-only / depth-trend on entry) or balance top-up to >=2 SOL before resuming.


## [2026-09-21 16:47 UTC] cron eval #41: bot still paused, no changes
- Solana bot: process NOT running. PAUSE_NEW_ENTRIES=True held since 2026-09-18 09:08 UTC (copy-trading pivot). Only base_memecoin/base_bot.py active on separate Base project.
- All-time state (unchanged from eval #40): **3574 trades**, **+25.485 SOL paper PnL** (inflated by slippage-simulation gap — treat as upper bound, not real PnL), **45.2% WR** (1617W / 1866L), **1.140566 SOL balance**, **0 open positions**. State.json mtime still 2026-09-20 00:20:23 UTC.
- Window since eval #40 (14:46 → 16:47 UTC, ~2h): **0 new trades**. 38th consecutive zero-trade eval under pause.
- Re-confirmed exit-category breakdown: tp_win 295, tp_partial 738, small_win 473, small_loss 801, rapid_loss 701, hardcap_loss 251, partial_loss 119, partial_profit 196. The 701 rapid losses + 251 hardcap losses = **952 rug stops (~27% of all trades)** — structural smoking gun unchanged.
- Post-reset window (Sep 16+): 54 trades / 5W / 49L / **9.3% WR** / **-0.859 SOL realized** (from starting_balance_sol 2.0 → balance_sol 1.140566). 47 of 49 losses are -100% ghost rugs on 0.02 SOL positions — **structural defect, not a parameter problem.**
- Decision: **NO CHANGES**. v8.7+ mechanical rules preserved (HARD_STOP_LOSS=-25%, MAX_POSITIONS=1, MAX_HOLD_MINUTES=30, TAKE_PROFIT_PCT=0.50, RESERVE_SOL=0.02). PAUSE_NEW_ENTRIES unchanged. v8.9 age filter 3.0 min unchanged. v9.4 liquidity floor 3.0 SOL unchanged. Awaiting structural fix (real fill-price verification / DEX-only / depth-trend on entry) or balance top-up to ≥2 SOL before resuming.

## [2026-09-21 18:49 UTC] cron eval #42: bot still paused, no changes
- Solana bot: process NOT running. PAUSE_NEW_ENTRIES=True held since 2026-09-18 09:08 UTC (copy-trading pivot). Only base_memecoin/base_bot.py active on separate Base project.
- All-time state (unchanged from eval #41): **3574 trades**, **+25.485 SOL paper PnL** (inflated by slippage-simulation gap — treat as upper bound, not real PnL), **45.2% WR** (1617W / 1866L / 91 breakeven), **1.140566 SOL balance**, **0 open positions**. state.json mtime **2026-09-20 00:20:23 UTC** (~42.5h stale — confirms no bot activity).
- Window since eval #41 (16:47 → 18:49 UTC, ~2h): **0 new trades**. 39th consecutive zero-trade eval under pause.
- Post-reset window (Sep 16+): 54 trades / 5W / 49L / **9.3% WR** / **-0.859 SOL realized** (from starting_balance_sol 2.0 → balance_sol 1.140566). 47 of 49 losses are -100% ghost rugs on 0.02 SOL positions — **structural defect, not a parameter problem.**
- Decision: **NO CHANGES**. v8.7+ mechanical rules preserved (HARD_STOP_LOSS=-25%, MAX_POSITIONS=1, MAX_HOLD_MINUTES=30, TAKE_PROFIT_PCT=0.50, RESERVE_SOL=0.02). PAUSE_NEW_ENTRIES unchanged. v8.9 age filter 3.0 min unchanged. v9.4 liquidity floor 3.0 SOL unchanged. Awaiting structural fix (real fill-price verification / DEX-only / depth-trend on entry) or balance top-up to ≥2 SOL before resuming.

## [2026-09-21 20:50 UTC] cron eval #43: bot still paused, no changes
- Solana bot: PAUSE_NEW_ENTRIES=True held since 2026-09-18 09:08 UTC (copy-trading pivot). Runner.log mtime is Sep 15 (stale). state.json mtime 2026-09-20 00:20:23 UTC (~44.5h stale).
- Bot running process per ps: `runner.py` (pid 294355, started Sep 16) is alive but produces "Buy blocked: would breach reserve (0.02 SOL)" — reservation gate is closed, no fills, no new trades since Sep 20. Confirming pause holds without intervention.
- All-time state: **3574 trades**, **+25.485 SOL paper PnL** (inflated by slippage-simulation gap — treat as upper bound, NOT real PnL), **45.2% WR** (1617W / 1866L / 91 breakeven), **1.140566 SOL balance**, **0 open positions**.
- Re-ran eval_window.py with last_eval=2026-09-17 00:23 UTC: 24 trades in window, **1W / 23L / 4.2% WR**, net **-0.4239 SOL realized**. **22 of 24 (91.7%) were GHOST exits** (-100% rugs, 0 SOL received). Strip those out → real exits n=2, 50% WR, -0.0039 SOL (essentially noise).
- Last 10 trades are all -100% GHOST rugs at 0.02 SOL each. Cause: pump.fun snipers drain bonding-curve liquidity between our buy and sell ticks; pool goes to 0; bot records ghost exit instead of phantom profit.
- Post-reset PnL still -0.859 SOL from starting_balance 2.0 → 1.1406 balance. window since eval #42 (18:49 → 20:50 UTC, ~2h): 0 new trades. 40th consecutive zero-trade eval under pause.
- Decision: **NO CHANGES**. v8.7+ mechanical rules preserved (HARD_STOP_LOSS=-25%, MAX_POSITIONS=1, MAX_HOLD_MINUTES=30, TAKE_PROFIT_PCT=0.50, RESERVE_SOL=0.02). PAUSE_NEW_ENTRIES unchanged. v8.9 age filter (3.0 min) unchanged. v9.4 bonding-curve liquidity floor (3.0 SOL) unchanged. Bot correctly idling; structural defect (sniper drain) needs design-level fix (DEX-only switch / depth-trend at entry / real fill-price verification), not a parameter knob.

## [2026-09-21 22:52 UTC] cron eval #44: bot still paused, no changes
- Solana bot: PAUSE_NEW_ENTRIES=True held since 2026-09-18 09:08 UTC (copy-trading pivot). Runner still alive (pid 294355, started Sep 16) but produces "Buy blocked: would breach reserve" — no fills, no new trades under pause. state.json mtime 2026-09-20 00:20:23 UTC (~50.3h stale).
- All-time state (unchanged from evals #41-#43): **3574 trades**, **+25.4850 SOL paper PnL** (slippage-simulation gap — upper bound, NOT real PnL), **46.4% WR** (1617W / 1866L / 91 breakeven), **1.140566 SOL balance**, **0 open positions**.
...
## [2026-09-22 02:56 UTC] cron eval #46: bot still paused, no changes
- Solana bot: PAUSE_NEW_ENTRIES=True held since 2026-09-18 09:08 UTC. Runner process (pid 294355) alive but blocked by both (a) PAUSE_NEW_ENTRIES gate and (b) LLM JSON parse failures starting 2026-09-19 23:21 UTC and continuing every tick through Sep 20 00:19:34 (last state mtime).
- All-time state (unchanged from eval #45): **3574 trades**, **+25.4850 SOL paper PnL** (slippage-simulation gap — upper bound, NOT real PnL), **45.2% WR** (1617W / 1866L / 91 breakeven), **1.140566 SOL balance**, **0 open positions**. state.json mtime 2026-09-20 00:20:23 UTC (~2h 36m stale — no writes since LLM parse failures started).
- Window since eval #45 (Sep 22 00:53 → 02:56 UTC, ~2h): **0 new trades**. **43rd consecutive zero-trade eval under pause.**
- Post-reset window (Sep 16+): 54 trades / 5W / 49L / **9.3% WR** / **-0.859 SOL realized** (from starting_balance 2.0 → balance 1.1406). 47 of 49 losses are -100% ghost rugs on 0.02 SOL positions — structural defect unchanged.
- **Two orthogonal blockers identified**:
  1. PAUSE_NEW_ENTRIES=True (manual halt, cron eval 2026-09-18 09:08 UTC) — by design.
  2. LLM JSON parse failures every tick since Sep 19 23:21 UTC — would block trading even if unpaused. Bot logs "observe" only; root cause likely model output format drift (markdown preamble, code-fence variations, missing closing braces). `extract_json` regex at bot.py:445-478 should handle but is failing.
- Decision: **NO CHANGES**. v8.7+ mechanical rules preserved (HARD_STOP_LOSS=-25%, MAX_POSITIONS=1, MAX_HOLD_MINUTES=30, TAKE_PROFIT_PCT=0.50, RESERVE_SOL=0.02). PAUSE_NEW_ENTRIES unchanged. v8.9 age filter 3.0 min unchanged. v9.4 bonding-curve liquidity floor 3.0 SOL unchanged. Structural fix (real fill-price verification / DEX-only / depth-trend on entry) still pending. LLM JSON parse failure is the next actionable investigation if user wants to unpause.
